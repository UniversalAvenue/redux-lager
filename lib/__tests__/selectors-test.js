...
var roles = {
  items: (0, _normalizr.arrayOf)(role)
};
...
function rolesAction(input, response) {
  return actionWith({
    input: input,
    identifier: input,
    schemaKeys: _lodash2.default.keys(roles),
    response: response
  });
}
...
var rolesResponse = (0, _normalizr.normalize)({
  items: [{ id: 1, name: 'Worker' }, { id: 2, name: 'Boss' }]
}, roles);
...
describe('inflatedResultSelector', function () {
  var rolesState = {
    lager: reducer({}, rolesAction('admin/roles', rolesResponse))
  };
  var selector = lager.inflatedResultSelector('admin/roles', roles);
  var m1 = selector(rolesState);
  var m2 = selector(rolesState);
  it('should memoize the output', function () {
    expect(m1).toEqual(m2);
  });
  it('should have a fully inflated result', function () {
