...
exports.inflatedResultSelector = inflatedResultSelector;
...
var _entitiesSelector = function _entitiesSelector(type) {
...
var entitiesSelector = exports.entitiesSelector = function entitiesSelector(type) {
  return _entitiesSelector(type.getKey ? type.getKey() : type);
};

var entitySelector = exports.entitySelector = function entitySelector(id, schema) {
  return function (state) {
    return _lodash2.default.get(state, 'lager.entities.' + schema.getKey() + '[' + id + ']');
  };
};
...
function inflatedResultSelector(identifier, schema) {
  var entityTypes = (0, _inflatr.getEntityKeys)(schema);
  var entitySelectors = entityTypes.map(function (t) {
    return entitiesSelector(t);
  });
  return (0, _reselect.createSelector)([resultSelector(identifier, 'result')].concat(_toConsumableArray(entitySelectors)), function (result) {
    ...
    return (0, _inflatr.inflate)(result, schema, getEntity)();
  });
}
...
function rowGetterSelector(identifier, schema) {
  return (0, _reselect.createSelector)(paginationSelector(identifier), inflatedEntitySelector(schema), function (store, getEntity) {
    return function (rowIndex) {
      ...
      return _lodash2.default.reduce(schema, function (sum, sc, key) {
        return (0, _updeep2.default)(_defineProperty({}, key, page[key] && getEntity(page[key][pagePosition], sc.getItemSchema())), sum);
      }, {});
    };
  });
}
